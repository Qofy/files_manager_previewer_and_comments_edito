...
import { foldersStorage } from '../+server.js';

const JWT_SECRET = 'your-secret-key-change-in-production';

// Import files storage (you'll need to export it from /src/routes/files/+server.js)
// For now, we'll create a reference to it
let filesStorage;
try {
  const filesModule = await import('../../files/+server.js');
...
function getAllChildFolderIds(folderId, username) {
  ...
  const directChildren = Array.from(foldersStorage.values())
    .filter(folder => folder.parent_id === folderId && folder.owner === username);
...
export async function GET({ request, url }) {
  ...
  const filesInFolder = Array.from(filesStorage.values())
    .filter(file => file.owner === username && allFolderIds.includes(file.folder_id));
